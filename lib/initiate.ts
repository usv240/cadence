...
export async function initiate({ transcript, styleCard, profile, memory, settings, keyword, n = 4 }: InitiateInput): Promise<InitiateOutput> {
  ...
  const response = await getOpenAIClient().responses.create({
    ...modelConfig,
    input: [
      { role: "system", content: "Help <user> START a live conversation on their own terms. Propose N short openers in the user's learned voice, grounded in their saved profile, local memory, conversation mode, language, and any recent transcript. Profile names and facts belong only to <user>; never assign them to another person mentioned in the transcript. Respect stated people, topic, and phrasing boundaries. Preserve the user's dialect, code-switching, and wording when preserveWording is true; never translate or normalize wording unless asked. Do not merely react. Spread options across sharing news, asking about the other person, raising a concern, opening a story, and expressing affection. Use memory and profile facts only when natural and never invent them. If a keyword is provided, use it naturally. Every opener needs a distinct opening word and correct sentence case. Return only the schema." },
